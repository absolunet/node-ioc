...
class Resolver {
	...
	static get dependencies() {
		return ['config', 'file', 'file.engine', 'path.view'];
	}
	...
	getViewPath(view) {
		return this.file.findFirst(this.extensions.map((extension) => {
			return path.join(this.getViewBasePath(view), `${view.replace(/^.+::/u, '').replace(/\./gu, path.sep)}.${extension}`);
		}));
	}
