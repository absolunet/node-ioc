...
class TestRepository {
	 ...
	 */
	fromNamespace(name = '') {
		return this.getFilesFrom(name).map((file) => {
	 ...
	 */
	getFilesFrom(folder) {
		const directory = path.join(this.basePath, this.scope, folder);
	...
	getNamespaceFromFile(file) {
		return path.relative(this.basePath, file).replace(/\/[^/]+.js$/, '')
	}
	...
	getFormattedNamespaceFromFile(file) {
		return title(this.getNamespaceFromFile(file).replace(/\//g, ' > '));
	}
	...
	getFormattedNameFromFile(file) {
		return file.replace(/^.*[\/\\](?<name>[A-z]+).js$/, '$<name>');
	}
	...
	getAllInstanceTestMethods(instance) {
		...
		do {
			Object.entries(Object.getOwnPropertyDescriptors(currentObject)).forEach(([name, descriptor]) => {
				if (this.testMethodName(name)) {
					properties.add(name);
				}
			});
