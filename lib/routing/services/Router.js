...
class Router {
	...
	static get dependencies() {
		return ['app', 'config', 'server', 'router.route', 'router.controller'];
	}
		...
		const data = [...__(this).get('groups'), attributes]
			.reduce((route, { as, prefix, path, namespace, action }) => {
				...
				const formattedPath = (prefix || path || '')
					...
					.join('/');
				route.path += formattedPath ? `/${formattedPath}` : '';

				const formattedNamespace = (namespace || '')
					.replace(/(?<last>\w)$/u, `$<last>${this.controllers.namespaceSeparator}`);
				route.action += formattedNamespace || (typeof action === 'string' ? action : '') || '';
	...
	generate() {
		...
		if (!__(this).get('generated')) {
			this.routes.all().forEach(({ constraints, defaults, path, action, method }) => {
				server[method](this.resolvePath(path, constraints), async (request, response) => {
					...
					controller.prepareHandling(this.app, request, response);

					try {
						if (typeof controller[controllerMethod] !== 'function') {
							const controllerName = this.controllers.getControllerName(action);

							throw new TypeError(`Action "${controllerMethod}" in controller "${controllerName}" does not exists.`);
						}

						await Promise.race([
							controller[controllerMethod](defaults),
						...
						exceptionHandler.render(error, request, response);
					}
	...
	callByName(name, parameters = {}, request = {}) {
		...
		const path = route.compilePath(parameters).getAttribute('compiledPath');
		const method = route.getAttribute('method');
