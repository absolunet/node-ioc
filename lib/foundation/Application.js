...
class Application extends Container {
	...
	boot() {
		...
		this.make('event').emit('application.booting', this);
	...
	configureDefaultPaths() {
		...
		this.configurePaths({
			...
			'controller': path.join(basePath, 'lib', 'http', 'controllers'),
	...
	flush() {
		...
		__(this).get('providers').forEach((provider) => {
	 ...
	 */
	onBooting(callback) {
		this.make('event').on('application.booting', callback);
	...
	onBooted(callback) {
		...
		return this.make('event').on('application.booted', callback);
	}
