 ...
 * @hideconstructor
 */
class VendorPublishCommand extends _Command.default {
  /**
  ...
  async handle() {
    const published = [];
    (0, _privateRegistry.default)(this).set('published', published);
    (0, _privateRegistry.default)(this).set('confirmQueue', []);
  ...
  async publishDirectory(from, to) {
    const files = await this.fs.scandir(from, 'file', {
      recursive: true,
      fullPath: true
    });
  ...
  async publishFile(from, to) {
    ...
    (0, _privateRegistry.default)(this).get('published').push({
      from,
      to
    });
  }
  ...
  async getPublishable() {
    let publishable = null; // If the user asked by CLI flag to have all publishable, no need to go through the whole process.

    if (this.flag('all')) {
      return _ServiceProvider.default.getAllPublishable();
    }
    ...
    if (provider) {
      ...
      publishable = _ServiceProvider.default.getPublishableByProvider(publishableProviders[provider]);
    ...
    const tagged = _ServiceProvider.default.getPublishableByTag(tag); // If a publishable and a tag were given, filter all provider's publishable
  ...
  async enqueueOverwriteConfirm(to) {
    const queue = (0, _privateRegistry.default)(this).get('confirmQueue');
    let answer;
    queue.push((async () => {
  ...
  getPublishableProviders() {
    return Object.fromEntries(_ServiceProvider.default.publishableProviders().map(value => {
  ...
  getPublishableTags() {
    return Object.fromEntries(_ServiceProvider.default.publishableTags().map(value => {
      return [value, value];
    }).sort(([a], [b]) => {
      return a.localeCompare(b);
    }));
...
var _default = VendorPublishCommand;
