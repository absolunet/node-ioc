...
class Router {
  ...
  redirect(from, to, permanent = false) {
    return this.any(from, this.withCorePrefix('RedirectController@handle')).with('to', to).with('permanent', permanent);
  }
  /**
  ...
  resource(resource, controller, only = [], apiOnly = false) {
    this.getResourceMapping(resource, controller).filter(({
      name,
      api
    }) => {
  ...
  getRoutePathMapping({
    ...
    const formattedPath = (prefix || path || '').split('/').filter(part => {
      return Boolean(part);
    }).join('/');
  ...
  getRouteActionMapping({
    ...
    const formattedNamespace = (namespace || '').replace(/(?<last>\w)$/u, `$<last>${this.controllers.namespaceSeparator}`);
  ...
  call(path, method = 'get', request = {}) {
    return this.runMiddleware(path, { ...request,
      method
    });
  }
  /**
  ...
  getResourceAction(action) {
    return this.getResourceActions().find(({
      name
    }) => {
      return name === action;
    });
  ...
  resolvePath(path, constraints) {
    return Object.entries(constraints).reduce((string, [parameter, constraint]) => {
