...
class Terminal extends _terminal.Terminal {
  ...
  async spawn(command, parameters, options) {
    await new Promise((resolve, reject) => {
      this.crossSpawn(command, parameters, options).on('close', code => {
        if (code === 0) {
          resolve();
        } else {
          reject(code);
  ...
  secret(question, options = {}) {
    return this.ask(question, null, { ...options,
      type: 'password'
    });
  }
  /**
  ...
  async confirm(statement, defaultValue = false) {
    const answer = await this.ask(statement, Boolean(defaultValue), {
      type: 'confirm'
    });
  ...
  table(header, data, options = {}, print = true) {
    const {
      table
    } = require('table'); // eslint-disable-line global-require
    ...
    const result = table(headerData.concat(tableData), Object.assign(defaultOptions, optionData)).replace(/\n. (?![a-zA-Z0-9]+).*\n/gu, '\n');
  ...
  tables(tables, sideBySide = false, options = {}, print = true) {
    ...
    if (sideBySide) {
      ...
      const higherLength = splitResults.reduce((l, {
        length
      }) => {
      ...
      for (let i = 0; i < higherLength; i++) {
        splitResults.forEach((rows, index) => {
